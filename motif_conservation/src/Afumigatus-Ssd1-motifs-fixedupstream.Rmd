...
```{r setwd_for_interactive_sessions_EWlaptop, include=FALSE,eval=FALSE}
setwd("~/Repos/calbicans_rna_motifs_2021/src/")
here::here()
```
...
Af_up1000_file <- paste(here::here(), "data", "Afumigatus_Af293_ATG_upstream_1000nt.fasta",
                              sep= "/")
...
id_file_CNYTCNYT1_up100 <- paste(here::here(), "results", "Af_id_list_CNYTCNYT1_up100.txt", sep = "/")
CNYTCNYT_count_df %>%
    filter(count_up100 >= 1) %>%
    pull(id) %>%
    write_lines(file = id_file_CNYTCNYT1_up100)

id_file_CNYTCNYT2_up100 <- paste(here::here(), "results", "Af_id_list_CNYTCNYT2_up100.txt", sep = "/")
CNYTCNYT_count_df %>%
