---
title: "Pileup Plots in R, for Ssd1 analysis"
...
Pileup plots for Ssd1 CRAC data. These show detailed profiles of read counts, including nucleotide-specific mutations and deletions, along selected transcripts. Search for instances of the `CNYUCNYU` Ssd1-enriched element in those transcripts.
...
```{r plot_SUN4focus,dependson = c("pileup_functions","load_onetable"),fig.height = 2,fig.width=4}
...
SUN4_5UTRwide_plot
# plot_nucseq(SUN4_5focus_df)
# ggsave(plot = SUN4_5UTRwide_plot, filename = "SUN4_5UTRwide_plot.pdf", 
#        height = 2, width=4, device = "pdf")
```

## Plot pileup focus on SUN4 5'UTR, restricted to "SUN4_duo" vicinity

```{r plot_SUN4dualSEE,dependson = c("pileup_functions","load_onetable"),fig.height = 2, fig.width=6}
...
SUN4_5dual_plot
ggsave(plot = SUN4_5dual_plot, filename = "SUN4_5dual_3_30_plot.pdf",
       height = 2, width=6, device = "pdf")
```

## Find all SEE positions in transcripts

Plot pileup on same area surrounding the SEE motifs (-12, +2) to look for consistent location of crosslinking sites.

```{r fig.height = 8, fig.width = 8}
SEE_positions <- position_locate_all(Ssd1_3_30_df) %>%
    as_tibble() %>%
...
# plot_pileup_one(SEE_motif_pileups,
#                 include = "allsubdel") + 
#     facet_wrap( ~ motifidg,scales = "free")  +
#             theme(axis.title.x =  element_blank(),
#                   axis.line.x = element_blank(),
#                   axis.text.x = element_blank(),
#                   axis.ticks.x = element_blank()) 
...
We conclude that we are likely to detect crosslinking by apparent deletions in U-rich regions 1-4nt upstream of CNYUCNYU. However, because htese are often U(4), or in some cases (CU)(2), the data do not tell us exactly which nucleotide position is crosslinked.
...
SUN4_5UTRwiderep_plot <- 
    ...
    labs(title = "SUN4 5'UTR peak, 30C rep A", x = "position from TSS")
SUN4_5UTRwiderep_plot
...
ggsave(plot = SRL1_5UTRupone_plot, 
       filename = "SRL1_5up_3_30_plot.pdf",
       height = 2, width = 12, device = "pdf")
...
ggsave(plot = SRL1_5UTRdownone_plot, 
       filename = "SRL1_5down_3_30_plot.pdf",
       height = 2, width = 12, device = "pdf")
```
